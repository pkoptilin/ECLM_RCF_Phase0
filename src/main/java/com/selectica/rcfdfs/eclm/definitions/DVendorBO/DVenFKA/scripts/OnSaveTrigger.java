package com.selectica.rcfdfs.eclm.definitions.DVendorBO.DVenFKA.scripts;

import com.selectica.rcfdfs.stubs.DVenDBA;
import com.selectica.rcfscripts.AbstractDataWriteScript;
import com.selectica.rcfdfs.stubs.DVenDetails;
import com.selectica.rcfdfs.stubs.DVenFKA;
import com.selectica.tools.StringUtils;
...
public class OnSaveTrigger extends AbstractDataWriteScript<Boolean> {
    /*
            <![CDATA[
				var fkaBussName = thisComponent.getBundleProxy().getParameterValueObjectsFromAnyMulticardComponent("ReqDVenFKA","fkaBussName");
				var str = "";
				for(i = 0; i < fkaBussName.length; i++) {
					str = str + fkaBussName[i] + ", ";
				}
				str = str.toString();
				var str1 = str.substring(0,str.length-2);
				thisComponent.setValue("DVenData/ReqDVenDetails/venFKA",str1);
    ...
    public Boolean process() throws Exception {
        ...
        String venFKA = "";
        for (DVenFKA fka : fkaList) {
            String fkaBussName = fka.getFkaBussName();
            venFKA = venFKA + fkaBussName + ", ";
        }
        if (venFKA.length() > 2)
            venFKA = venFKA.substring(0, venFKA.length() - 2);

        DVenDetails details = getHelper().getInfoComponentStub();
        details.setVenFKA(venFKA);
